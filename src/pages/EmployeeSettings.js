...
const SettingManager = ({ title, tableName, columns, items, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ({ error } = await supabase.from(tableName).update(updateData).eq('id', id));
      } else {
        ({ error } = await supabase.from(tableName).insert([updateData]));
      }
            ...
            <div className="settings-form-grid">
              {columns.map(col => (
                <div className="form-group" key={col.key}>
                  ...
                  <input type="text" name={col.key} value={currentItem[col.key] || ''} onChange={handleInputChange} required />
                </div>
              ))}
            </div>
...
function EmployeeSettings() {
  ...
  const jobCodeColumns = [
    { key: 'code', header: 'Job Code' }, { key: 'description', header: 'Description' },
    { key: 'pay_type', header: 'Pay Type' }, { key: 'eeoc_id', header: 'EEOC ID' },
